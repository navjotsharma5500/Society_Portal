...
const createTemplate = async () => {
  ...
  sheet.columns = [
    ...
    ["Entity Type*", 22], ["Campus*", 18], ["Category", 20], ["President Name", 25],
    ["President Email", 30], ["President Designation", 28], ["Academic Session*", 20], ["Active", 12],
  ].map(([header, width]) => ({ header, width }));
  sheet.getRow(1).font = { bold: true, color: { argb: "FFFFFFFF" } };
  sheet.getRow(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF1F4E78" } };
  sheet.addRow(["Example Technical Society", "", "example.society@example.edu", "SOCIETY", "PATIALA", "Society", "Example Person", "person@example.edu", "Faculty Coordinator", "2026-27", "TRUE"]);
  for (let row = 2; row <= 500; row += 1) {
    ...
    sheet.getCell(row, 11).dataValidation = { type: "list", allowBlank: false, formulae: ['"TRUE,FALSE"'] };
  }
  ...
  instructions.addRows([
    ...
    ["Leadership", "Multiple leadership records will be supported in v0.2.2."],
